...
export class CrearViajes {
  ...
  guardarViaje(user_id: string): Observable<any>{
    console.log("[src][app][services][MostrarUsers][home_users]")
    return this._httpclient.get<any>(this.supabaseURL+'VIAJE?user_id=eq.'+user_id, { headers: this.supabaseHttpHeaders, responseType: 'json'}).pipe(
      map( (data) => {
        console.log(data);
        return data;
      }),
